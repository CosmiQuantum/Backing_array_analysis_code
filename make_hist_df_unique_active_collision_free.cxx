...
struct ChanKey {
  ll board;
  ll ch;
  bool operator==(const ChanKey& o) const { return board == o.board && ch == o.ch; }
};
...
int main(int argc, char** argv) {
  if (argc < 3) {
    std::cerr << "Usage:\n  " << argv[0] << " input.root output.root [treeName]\n";
    return 1;
  }
  ...
  ll Event_Id=0, Board_Id=0, CH_Id=0, ToT_LSB=0;
  int Ring=-1;
  ...
  // Output accumulator: per ring counts of events with >=thr (thr=1..8)
  std::unordered_map<int, std::array<long long, 8>> hist_by_ring;
  for (int r=1; r<=28; r++) hist_by_ring[r] = {0,0,0,0,0,0,0,0};

  // For current event: per ring unique channel set
  std::unordered_map<int, std::unordered_set<ChanKey, ChanKeyHash>> uniq_by_ring;

  auto finalize_event = [&]() {
    // For each ring that appeared in this event, compute unique count and update hist
    for (auto& kv : uniq_by_ring) {
      int ring = kv.first;
      int cnt = (int)kv.second.size();
      for (int thr=1; thr<=8; thr++) {
        if (cnt >= thr) hist_by_ring[ring][thr-1] += 1;
      }
    }
    uniq_by_ring.clear();
  };
  ...
  if (n == 0) {
    std::cerr << "WARNING: input tree empty.\n";
  } else {
    ll current_event = -1;
    ...
    for (ll i=0; i<n; i++) {
      ...
      if (ToT_LSB <= 0) continue;
      if (Ring < 0) continue;

      if (current_event < 0) current_event = Event_Id;
      ...
      // insert unique channel for this ring in this event
  ...
  TTree* th = new TTree("hist_df", "hist_df: per ring event counts for >=thr unique channels");

  int out_ring=0;
  long long ge_1=0, ge_2=0, ge_3=0, ge_4=0, ge_5=0, ge_6=0, ge_7=0, ge_8=0;
  ...
  for (auto& kv : hist_by_ring) rings.push_back(kv.first);
  std::sort(rings.begin(), rings.end());

  for (int r : rings) {
    out_ring = r;
    const auto& a = hist_by_ring[r];
    ge_1 = a[0]; ge_2 = a[1]; ge_3 = a[2]; ge_4 = a[3];
    ge_5 = a[4]; ge_6 = a[5]; ge_7 = a[6]; ge_8 = a[7];
    th->Fill();
  }
  ...
  std::cout << "Wrote FAST collision-free hist_df to: " << outpath << "\n";
  return 0;
}
